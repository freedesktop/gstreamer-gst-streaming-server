...
static guint gss_websocket_signals[LAST_SIGNAL] = { 0 };
...
static void gss_websocket_finalize (GObject * object);
static void gss_websocket_dispose (GObject * object);

static void
gss_websocket_init (GssWebsocket * client)
{
  client->conn_id = next_conn_id++;
}

static void
gss_websocket_class_init (GssWebsocketClass * client_class)
{
  GObjectClass *gobject_class = (GObjectClass *) (client_class);

  gobject_class->dispose = gss_websocket_dispose;
  gobject_class->finalize = gss_websocket_finalize;

  gss_websocket_signals[CONNECTION_LOST] =
      g_signal_new ("connection-lost", G_TYPE_FROM_CLASS (client_class),
      G_SIGNAL_RUN_LAST, 0, NULL, NULL,
      g_cclosure_marshal_generic, G_TYPE_NONE, 0, G_TYPE_NONE);

  gss_websocket_signals[MSG_RECEIVED] =
...
static void
gss_websocket_finalize (GObject * object)
{
  GssWebsocket *client = (GssWebsocket *) (object);
  ...
  G_OBJECT_CLASS (gss_websocket_parent_class)->finalize (object);
}

static void
gss_websocket_dispose (GObject * object)
{
  GssWebsocket *client = (GssWebsocket *) (object);
  ...
  G_OBJECT_CLASS (gss_websocket_parent_class)->dispose (object);
}

static void
snra_server_connection_lost (GssWebsocket * client)
{
  if (client->fired_conn_lost)
    return;
  client->fired_conn_lost = TRUE;

  GST_INFO ("Lost connection for client %u", client->conn_id);
  ...
  if (client->type == GSS_WEBSOCKET_CHUNKED ||
      client->type == GSS_WEBSOCKET_SINGLE) {
    if (client->need_body_complete)
      soup_message_body_complete (client->event_pipe->response_body);
  ...
  g_signal_emit (client, gss_websocket_signals[CONNECTION_LOST], 0, NULL);
}

static void
gss_websocket_disconnect (G_GNUC_UNUSED SoupMessage * message,
    GssWebsocket * client)
{
  GST_INFO ("client %u disconnect signal", client->conn_id);
  client->need_body_complete = FALSE;
...
#if 0
static void
gss_websocket_network_event (G_GNUC_UNUSED SoupMessage * msg,
    G_GNUC_UNUSED GSocketClientEvent event,
    G_GNUC_UNUSED GIOStream * connection, G_GNUC_UNUSED GssWebsocket * client)
{
...
try_parse_websocket_fragment (GssWebsocket * client)
{
  ...
  GST_LOG ("Got %u bytes to parse", (guint) client->in_bufavail);
  ...
  if (header[0] & 0x80)
    GST_LOG ("FIN flag. Payload type 0x%x", header[0] & 0xf);
  ...
  if ((header[1] & 0x80) == 0) {
    ...
    GST_LOG ("Received packet not masked. Skipping");
    snra_server_connection_lost (client);
    goto skip_out;
  }
  ...
  g_signal_emit (client, gss_websocket_signals[MSG_RECEIVED], 0,
...
static gboolean
gss_websocket_io_cb (G_GNUC_UNUSED GIOChannel * source,
    GIOCondition condition, GssWebsocket * client)
{
  GIOStatus status = G_IO_STATUS_NORMAL;

  GST_LOG ("Got IO callback for client %p w/ condition %u", client,
      (guint) (condition));
  ...
  if (condition & G_IO_IN) {
    ...
    if (client->in_bufsize <= client->in_bufavail) {
      ...
      client->in_bufsize *= 2;
      GST_LOG ("Growing io_buf to %" G_GSIZE_FORMAT " bytes",
          client->in_bufsize);
    ...
    if (status == G_IO_STATUS_ERROR) {
      ...
      GST_LOG ("Collected %" G_GSIZE_FORMAT " bytes to io buf", bread);
...
static void
gss_websocket_wrote_headers (SoupMessage * msg, GssWebsocket * client)
{
  GList *cur;
  ...
  GST_LOG ("client %u ready for traffic", client->conn_id);
  ...
  client->io_watch = g_io_add_watch (client->io, G_IO_IN | G_IO_HUP,
      (GIOFunc) (gss_websocket_io_cb), client);
  ...
  while ((cur = client->pending_msgs)) {
    ...
    gss_websocket_send_message (client, msg->body, msg->len);
...
is_websocket_client (GssWebsocket * client)
{
  ...
  GST_LOG ("WebSocket connection with protocol %d", protocol_ver);
...
{
  GssWebsocket *client = g_object_new (GSS_TYPE_WEBSOCKET, NULL);
  ...
  client->net_event_sig = g_signal_connect (msg, "network-event",
      G_CALLBACK (gss_websocket_network_event), client);
#endif
  client->disco_sig = g_signal_connect (msg, "finished",
      G_CALLBACK (gss_websocket_disconnect), client);

  if (!is_websocket_client (client)) {
    client->type = GSS_WEBSOCKET_CHUNKED;
  ...
  client->type = GSS_WEBSOCKET_WEBSOCKET;
  client->need_body_complete = FALSE;
  ...
  client->wrote_info_sig = g_signal_connect (msg, "wrote-informational",
      G_CALLBACK (gss_websocket_wrote_headers), client);
...
static void
write_fragment (GssWebsocket * client, gchar * body, gsize len)
{
  gchar header[14];
  gsize header_len, i;
...
void
gss_websocket_send_message (GssWebsocket * client, gchar * body, gsize len)
{
  PendingMsg *msg;
  ...
  if (client->type == GSS_WEBSOCKET_SINGLE) {
...
const gchar *
gss_websocket_get_host (GssWebsocket * client)
{
  return client->host;
}
